...
class Activity3Test(StaticLiveServerTestCase):
    ...
    def test_reviews_logo_get(self):
        """
        Test that the reviews' logo2.png can be downloaded, and the content matches that on disk. This also checks the
        logo2.png is in the right location and is being served using the static files' finder.
        """
        response = urlopen(self.live_server_url + '/static/reviews/logo2.png').read()
        current_dir = os.path.dirname(os.path.abspath(__file__))
        with open(os.path.join(current_dir, 'static', 'logo2.png'), 'rb') as f:
            self.assertEqual(response, f.read())

    def test_main_logo_get(self):
        """
        Test that the main logo2.png can be downloaded, and the content matches that on disk. This also checks the
        logo2.png is in the right location and is being served using the static files' finder.
        """
        response = urlopen(self.live_server_url + '/static/logo2.png').read()
        with open(os.path.join(settings.BASE_DIR, 'static', 'logo2.png'), 'rb') as f:
            self.assertEqual(response, f.read())
    ...
    def test_base_html_content(self):
        ...
        tag, main logo2 in brand block and no <style>...</style> tags.
        """
        ...
        self.assertIn(
            '<a class="navbar-brand" href="/">{% block brand %}<img src="{% static \'logo2.png\' %}">{% endblock %}</a>',
            base_template)
    ...
    def test_base_logo_display(self):
        ...
        c = Client()
        resp = c.get('/')
        self.assertIn(b'<a class="navbar-brand" href="/"><img src="/static/logo2.png"></a>', resp.content)

    def test_review_base_extends_main(self):
        """Test that the reviews base.html extends from the base.html"""
